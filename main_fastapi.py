# main_fastapi.py
# Versión multibot Twilio ↔ OpenAI Realtime (baseline estable + barge-in simple)
...
# - MATRIZ por JSON en bots/<slug>.json
# - Cambio mínimo: barge-in simple (si el usuario habla, cancelamos el TTS). Sin commits forzados.
...
async def media_socket(websocket: WebSocket):
    ...
    # ---- Estado de conversación (barge-in simple) ----
    ...
    try:
        async with websockets.connect(
            ...
            async def twilio_to_openai():
                nonlocal stream_sid, speaking_out, barge_cancel_sent
                try:
                    while True:
                        ...
                        if ev == "start":
                            ...
                            # BAR­GE-IN SIMPLE: si el bot está hablando y entra voz del usuario, cancelamos TTS
                            ...
                            # Pasar audio del usuario tal cual
                            await openai_ws.send(json.dumps({
            ...
            async def openai_to_twilio():
                ...
                try:
                    async for raw in openai_ws:
                        ...
                        # VAD: cuando el modelo detecta que el usuario acabó de hablar → commit + respuesta
                        if t == "input_audio_buffer.speech_stopped":
                            print("✂️  [OpenAI] speech_stopped → commit + response.create")
                            await openai_ws.send(json.dumps({"type": "input_audio_buffer.commit"}))
                            await openai_ws.send(json.dumps({"type": "response.create"}))
                            # Reset del barge-in para la siguiente vuelta
                        ...
                        if t in ("response.output_item.done", "response.done"):
                            speaking_out = False
                            # barge_cancel_sent se resetea cuando llegue el próximo speech_stopped
